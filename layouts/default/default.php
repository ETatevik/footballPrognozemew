    ...
    <body>
        ...
        <article class="indexContentBody row">
            ...
            <section id="pageContent">
                <div class="container">
                    <!-- start slider in index page  -->
                    <div id="indexPageSilder">
                       <?php foreach($cnt->getDataUser("f_slider", "WHERE `status`='1'", "all") as $slide) {
                            // get data slide
                            $slide_id = $slide['id'];
                            $slide_title = $slide['title_ru'];
                            $slide_descr = trim(htmlspecialchars($slide['descr_ru']));
                            $slide_link = $slide['link'];
                            // get slide img 
                            $slide_image = $cnt->getDataUser("files", "WHERE `table_name`='f_slider' AND `name_used`='slide' AND `row_id`='$slide_id'");
                            if ($slide_image) {
                                $slide_img_name = $slide_image['name'];
                                $slide_img_type = $slide_image['type'];
                                $slide_show = '/public/img/f_slider/large/'.$slide_img_name.'.'.$slide_img_type;
                            }else {
                                $slide_show = '/public/img/no_photo.jpg';
                            }
                        ?>
                        <div class="slide active" style="background-image: url('<?=$slide_show?>')">
                            <div class="container-fluid">
                                <h1><?=$slide_title?></h1>
                                <p><?=$slide_descr?></p>
                                <a href="<?=$slide_link?>" target="_blank"><button class="btn moreInfo" type="button"><span>ПОДРОБНЕЕ</span></button></a>
                            </div>
                        </div>
                        <?php }?>
                    </div>
                    <!-- end slider in index page  -->
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-foot order-mb-2">
                                    <ul class="row">
                                       <?php foreach($cnt->getDataUser("f_contacts", "", "all") as $contacts) {
                                            // social links
                                            $social_links_show = '';
                                            ...
                                            if ($contacts['social_links'] != '') {
                                                ...
                                                foreach($social_links as $sl_id => $sl_link) {
                                                    $social_links_show = $sl_link;
                                                }
                                            }
                                        ?>
                                        <li>
                                            <a href="<?=$social_links_show?>">
