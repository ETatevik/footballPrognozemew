<?php
if(!isset($_GET['id'])) {
    header('location: /default/pages/news');
    exit;
} else {
    $id = $_GET['id'];
}
        ...
        <article class="indexContentBody row">
                ...
                <div class="container">
                    ...
                    <div id="nav-node-list">
                        <ul class="row">
                            ...
                            <li class="text-nowrap">
                                <span>Евро-2020: Нидерланды победили Германию в матче с шестью голами: сырые немцы, невероятные «оранье» и важность VAR</span>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div id="newsIdBox"> 
                        <div id="newsArticleBody">
                            <?php
                            // get news data
                            $news = $cnt->getDataUser("f_news", "WHERE `id`='$id'");
                            $news_id = $news['id'];
                            $news_title = $news['title_ru'];
                            $news_descr = decodeText($news['descr_ru']);
                            ...
                            if ($news_image) {
                                $news_img_name = $news_image['name'];
                                $news_img_type = $news_image['type'];
                                $news_img_show = '/public/img/f_news/large/'.$news_img_name.'.'.$news_img_type;
                            } else {
                                $news_img_show = '/public/img/no_photo.jpg';
                            }
                            ?>
                            <p class="news-date"><?=$news_date?></p>
                            <div class="news-Id-Info">
                                <h1 class="news-head"><?=$news_title?></h1>
                                <img src="<?=$news_img_show?>" alt="" class="news-img">
                                <p class="news-text"><?=$news_descr?></p>
                            </div>
                        <?php ?>
                        </div>
                        <div id="recommendedReadingBox">
                            <h6>Рекомендуем к прочьтению</h6>
                            <div class="recommendBlockSlide">
                                <div class="item">
                                    <ul id="recommend-slider" class="recommend-slider">
                                        <?php
                                        // get news
                                        foreach($cnt->getDataUser("f_news", "WHERE `status`='1'", "all") as $all_news) {
                                            $all_news_id = $all_news['id'];
                                            $all_news_title = $all_news['title_ru'];
                                            $all_news_descr = $all_news['descr_ru'];
                                            $all_news_date = date('d.m.Y H:i', strtotime($all_news['date']));
                                            $all_news_image = $cnt->getDataUser("files", "WHERE `table_name`='f_news' AND `name_used`='image' AND `row_id`='$all_news_id'");
                                            if ($all_news_image) {
                                                $all_news_img_name = $all_news_image['name'];
                                                $all_news_img_type = $all_news_image['type'];
                                                $all_news_img_show = '/public/img/f_news/large/'.$all_news_img_name.'.'.$all_news_img_type;
                                            } else {
                                                $all_news_img_show = '/public/img/no_photo.jpg';
                                            }    
                                            ?>
                                            <li>
                                                <div class="col-news">
                                                    <a href="/pages/newsID?id=<?=$all_news_id?>" class="news-card">
                                                        <div class="card-img">
                                                            <img src="<?=$all_news_img_show?>">
                                                        </div>
                                                        <div class="card-body">
                                                            <h5><?=$all_news_title?></h5>
                                                            <p><?=$all_news_date?></p>
                                                        </div>
                                                    </a>
                                                </div>
                        ...
                        <div class="detailsForecastBox" id="moreOfForecast">
                            <div class="more-block">
                                <p>Детали к прогнозу</p>
                            </div>
                            <div class="moreInfoForecBox">
                                <div class="defLig">
                                    <h6>
                                        ...
                                        <span>ОСНОВНАЯ ЛИГА</span>
                                    </h6>
                                </div>
                                <div class="row">
                                    <div class="col-lig-forec">
                                        <p>Ливерпуль</p>
                                        <img src="/public/img/tabelForecastsLiga/st1.png" alt="Start Team">
                                    </div>
                                    ...
                                    <div class="col-lig-forec">
                                        <p>Тоттенхэм</p>
                                        <img src="/public/img/tabelForecastsLiga/en1.png" alt="End Team">
                                    </div>
                                </div>
                                <table>
                                    <tr>
            ...
            <section id="successForecasts">
                ...
                <div class="container-row">
                    ...
                    <?php include('layouts/default/inc/right_side.php')?>
                    </div>
            </section>
        </article>
        <!-- end page content -->
